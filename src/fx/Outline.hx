...
class Outline extends Shader<InternalShader> {
	...
	static var SRC = {
		...
		function fragment() {
            ...
            if(curColor.a < 1) {
                ...
                var d : Vec4 = texture.get(uv + radius * vec2(0, texelSize.y));
                var r : Vec4 = texture.get(uv + radius * vec2(texelSize.x, 0));
                var ul : Vec4 = texture.get(uv - radius * vec2(texelSize.x, texelSize.y));
                var ur : Vec4 = texture.get(uv + radius * vec2(texelSize.x, -texelSize.y));
                var dl : Vec4 = texture.get(uv + radius * vec2(-texelSize.x, texelSize.y));
                var dr : Vec4 = texture.get(uv + radius * vec2(texelSize.x, texelSize.y));
				var dark = (u.a > 0 ? intensity : 0) + (l.a > 0 ? intensity : 0) + (d.a > 0 ? intensity : 0) + (r.a > 0 ? intensity : 0) + (ul.a > 0 ? intensity : 0) + (ur.a > 0 ? intensity : 0) + (dl.a > 0 ? intensity : 0) + (dr.a > 0 ? intensity : 0);
